...
from qmines.utilities.constants import BOARD_MIN_LENGTH, BOARD_MAX_LENGTH, EASY_SETTINGS
    

class GameParameters:
    SIZE_RANGE: Final[range] = range(BOARD_MIN_LENGTH, BOARD_MAX_LENGTH + 1)
    ...
    def from_dict(cls, dict_: Mapping[str, int], *,
        ...
        time_limit_ = (time_limit if time_limit is not None
                       else dict_.get('time_limit_in_seconds', EASY_SETTINGS['time_limit_in_seconds']))
    ...
    def _verify_length(cls, length: int) -> None:
        if length not in cls.SIZE_RANGE:
            raise ValueError(f'Length {length} must be in range [{BOARD_MIN_LENGTH}, {BOARD_MAX_LENGTH}].')
